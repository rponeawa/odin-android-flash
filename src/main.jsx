...
function App(){
 ...
 const [adb,setAdb]=useState(null),[message,setMessage]=useState('');
 useEffect(()=>{fetch(REPO).then(r=>r.json()).then(xs=>setReleases(xs.filter(x=>!/(底包|base|firmware)/i.test(x.name||'')&&x.assets?.some(a=>/\.part-\d+$/.test(a.name))))).catch(e=>setMessage(e.message));},[]);
 const connect=async()=>{setBusy('连接设备');try{const a=await connectAdb();setAdb(a);setMessage(`已连接 ${a.serial}`);setStep(1);}catch(e){setMessage(e.message)}finally{setBusy('')}};
