...
async function sendSideload(adb,source,onProgress,total){
  ...
  const writer=socket.writable.getWriter(); const reader=socket.readable.getReader(); let sent=0; let pending=new Uint8Array();
  const readExact=async n=>{while(pending.length<n){const x=await reader.read();if(x.done)throw Error('sideload 连接已断开');const z=new Uint8Array(pending.length+x.value.length);z.set(pending);z.set(x.value,pending.length);pending=z;}const out=pending.slice(0,n);pending=pending.slice(n);return out;};
  const file=source instanceof Blob?source:null; const get=async(offset,len)=>{if(file)return new Uint8Array(await file.slice(offset,offset+len).arrayBuffer());return source(offset,len)};
  while(true){const cmd=new TextDecoder().decode(await readExact(8));if(cmd==='DONEDONE')break;if(cmd==='FAILFAIL')throw Error('TWRP 拒绝刷机包');const block=Number(cmd);if(!Number.isInteger(block))throw Error(`sideload 返回无效块号 ${cmd}`);const offset=block*262144;const len=Math.min(262144,total-offset);if(len<=0)throw Error('sideload 请求超出文件范围');const data=await get(offset,len);await writer.write(data);sent=Math.max(sent,offset+data.length);onProgress(sent,total);}await writer.close();await reader.cancel();await socket.close();
}
...
function App(){
 ...
 const flash=async()=>{if(!adb||!rom)return;setBusy('下载并刷入');try{const assets=rom.assets.filter(a=>/\.part-[ab]-\d+$/.test(a.name)).sort((a,b)=>a.name.localeCompare(b.name,undefined,{numeric:true}));const total=assets.reduce((n,a)=>n+Number(a.size||0),0);let downloaded=0;const source=async(offset,len)=>{let base=0;for(const a of assets){const size=Number(a.size||0);if(offset<base+size){const local=offset-base;const r=await fetch(a.browser_download_url,{headers:{Range:`bytes=${local}-${local+len-1}`}});if(!r.ok&&r.status!==206)throw Error(`下载分卷失败 HTTP ${r.status}`);const data=new Uint8Array(await r.arrayBuffer());downloaded=Math.max(downloaded,offset+data.length);setProgress({label:`下载并刷入 ${Math.round(downloaded/total*100)}%`,done:downloaded,total});return data}base+=size}throw Error('刷机包偏移超出分卷范围')};await sendSideload(adb,source,(d,t)=>setProgress({label:'刷入刷机包',done:d,total:t}),total);await adb.power.reboot();setStep(5);setMessage('刷机包已刷入，设备正在重启');}catch(e){setMessage(e.message)}finally{setBusy('');setProgress(null)}};
