...
const url=process.env.MEGA_FOLDER_URL, start=Number(process.env.RANGE_START), end=Number(process.env.RANGE_END), out=process.env.OUTPUT, nameOut=process.env.NAME_OUTPUT, sizeOut=process.env.SIZE_OUTPUT;
...
const decodedName=decodeURIComponent(file.name); if(nameOut) fs.writeFileSync(nameOut, decodedName); if(sizeOut) fs.writeFileSync(sizeOut, String(file.size)); console.log(JSON.stringify({name:decodedName,size:file.size,start,end}));
