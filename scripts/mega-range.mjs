...
if(!url) throw Error('MEGA_FOLDER_URL required');
...
if(side){const midpoint=Math.ceil(file.size/2); start=side==='a'?0:midpoint; end=side==='a'?midpoint-1:file.size-1;} const decodedName=decodeURIComponent(file.name); if(nameOut) fs.writeFileSync(nameOut, decodedName); if(sizeOut) fs.writeFileSync(sizeOut, String(file.size)); console.log(JSON.stringify({name:decodedName,size:file.size,start,end}));
// 不给 OUTPUT 就只报告最新的名字与大小，不下载——用来判断这个包是否已经发布过
if(!out){ process.exit(0); }
if(!Number.isFinite(start)||!Number.isFinite(end)) throw Error('RANGE_SIDE or RANGE_START/RANGE_END required');
