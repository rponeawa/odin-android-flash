...
for await (const chunk of file.download()) { let off=0; hash.update(chunk); while(off<chunk.length){if(used===chunkSize){await new Promise((r,j)=>stream.end(e=>e?j(e):r()));open()} const n=Math.min(chunk.length-off,chunkSize-used); if(!stream.write(chunk.subarray(off,off+n))) await new Promise(r=>stream.once('drain',r)); off+=n; used+=n; }}
