...
const hash=crypto.createHash('sha256'); let index=0, used=0, stream=null, total=0, nextLog=100*1024*1024;
...
for await (const chunk of file.download({forceHttps:true,maxConnections:8,initialChunkSize:4*1024*1024,chunkSizeIncrement:4*1024*1024,maxChunkSize:16*1024*1024})) { let off=0; hash.update(chunk); total += chunk.length; if (total >= nextLog) { console.log(`downloaded ${Math.round(total/1024/1024)} MiB`); nextLog += 100*1024*1024; } while(off<chunk.length){if(used===chunkSize){await new Promise((r,j)=>stream.end(e=>e?j(e):r()));open()} const n=Math.min(chunk.length-off,chunkSize-used); if(!stream.write(chunk.subarray(off,off+n))) await new Promise(r=>stream.once('drain',r)); off+=n; used+=n; }}
